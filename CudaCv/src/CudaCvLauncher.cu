...
void _Init(CUDA_CV_GLOBAL global, const int max_resolution_x, const int max_resolution_y, const int image_channels, const int max_gaussian_kernel_size)
{
	...
	global.TotalSize      = max_resolution_x * max_resolution_y * image_channels * sizeof(cvBYTE);
	...
	CreateGaussianKernels(global.FilterKernels, max_gaussian_kernel_size, 1.0);
