...
static double* GetGaussianKernel(int size)
{
	...
	if (global.BlurFilterKernels.DeviceKernels == 0) return 0;

	int i = 0;
	while (i <= global.BlurFilterKernels.Length)
	{
		if (global.BlurFilterKernels.DeviceKernels[i]->Width == size)
		{
			return global.BlurFilterKernels.DeviceKernels[i]->Kernel;
		}
		i++;
	}
	return NULL;
